...
const IconHammer = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 12l-8.5 8.5a2.12 2.12 0 0 1-3-3L12 9" /><path d="M17.64 15L22 10.64" /><path d="M20.91 11.7a5.56 5.56 0 0 0-7.61-7.61" /><path d="M14 8l2 2" /></svg>);
...
const IconSpin = () => (<svg className="spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12a9 9 0 1 1-6.219-8.56" /></svg>);
const IconWallet = () => (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2" /><path d="M2 10h20" /><circle cx="17" cy="15" r="1" fill="currentColor" /></svg>);
const IconExternal = () => (<svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>);
const IconCube = () => (<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z" /><polyline points="3.27 6.96 12 12.01 20.73 6.96" /><line x1="12" y1="22.08" x2="12" y2="12" /></svg>);
const IconUnlock = () => (<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 9.9-1" /></svg>);
...
// ── NEW: No-wallet banner ──────────────────────────────────────────────────
function WalletBanner({ onConnect }) {
  return (
    <div className="wallet-banner">
      <IconWallet />
      <span>Connect your Freighter wallet to interact with the contract</span>
      <button className="btn btn-primary" style={{ padding: "6px 14px", marginLeft: "auto" }} onClick={onConnect}>
        Connect
      </button>
    </div>
  );
...
export default function App() {
  ...
  // ── CHANGED: single shared agreementId used across Track / Claim / Void ──
  ...
  async function connectWallet() {
    try {
      if (FreighterApi && typeof FreighterApi.isConnected === "function") {
        ...
        if (!connected?.isConnected) { addToast("Freighter not connected — unlock the extension", "error"); return; }
  ...
  async function createEscrow() {
    ...
    if (!id || !payer || !receiver || !token || !amount || !milestones) { addToast("Fill all fields", "error"); return; }
    ...
    // ── CHANGED: auto-populate agreementId after forging so Track/Claim/Void are ready
    if (result) setAgreementId(id);
  }

  async function completeMilestone() {
    if (!agreementId || milestoneIdx === "") { addToast("Provide contract ID & phase index", "error"); return; }
    const op = contract.call(
      "approve_phase",
      StellarSdk.nativeToScVal(agreementId, { type: "symbol" }),
      StellarSdk.nativeToScVal(parseInt(milestoneIdx), { type: "u32" }),
    );
    await invoke(op);
    await fetchEscrow(agreementId);
  }

  async function releaseEscrow() {
    if (!agreementId) { addToast("Provide contract ID", "error"); return; }
    const op = contract.call("withdraw", StellarSdk.nativeToScVal(agreementId, { type: "symbol" }));
    await invoke(op);
  }

  async function cancelEscrow() {
    if (!agreementId) { addToast("Provide contract ID", "error"); return; }
    const op = contract.call("terminate", StellarSdk.nativeToScVal(agreementId, { type: "symbol" }));
    await invoke(op);
    // ── CHANGED: clear escrow state after termination
    setEscrow(null);
    setAgreementId("");
  }
  ...
  async function fetchEscrow(id) {
    ...
    try {
      ...
      const op = contract.call("get_agreement", StellarSdk.nativeToScVal(id, { type: "symbol" }));
      const tx = new StellarSdk.TransactionBuilder(account, {
        fee: StellarSdk.BASE_FEE, networkPassphrase: NETWORK_PASSPHRASE,
      }).addOperation(op).setTimeout(30).build();
  ...
  return (
    <>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body::before {
          ...
          background-image: repeating-linear-gradient(
            ...
            rgba(26,22,18,.025) 8px, rgba(26,22,18,.025) 9px
          );
          pointer-events: none;
          z-index: 0;
        }

        .app { position: relative; z-index: 1; max-width: 700px; margin: 0 auto; padding: 32px 16px 120px; }

        .header { display: flex; align-items: center; gap: 10px; margin-bottom: 36px; flex-wrap: wrap; }
        .logo { display: flex; align-items: center; gap: 10px; margin-right: auto; text-decoration: none; }
        .logo-mark { width: 38px; height: 38px; background: var(--ink); border-radius: var(--r); display: flex; align-items: center; justify-content: center; color: var(--bg); flex-shrink: 0; }
        .logo-text-wrap { display: flex; flex-direction: column; gap: 0; }
        .logo-name { font-size: 1.05rem; font-weight: 900; font-variation-settings: 'wdth' 110; letter-spacing: -.03em; color: var(--ink); line-height: 1; text-transform: uppercase; }
        .logo-sub { font-family: var(--mono); font-size: .58rem; color: var(--ink3); letter-spacing: .12em; text-transform: uppercase; line-height: 1; margin-top: 2px; }
        .net-badge { font-family: var(--mono); font-size: .62rem; font-weight: 700; color: var(--orange); background: var(--orange-dim); border: 1px solid var(--orange)44; padding: 3px 8px; border-radius: var(--r); letter-spacing: .06em; text-transform: uppercase; margin-left: 2px; }

        .hdr-btn { display: inline-flex; align-items: center; gap: 6px; padding: 7px 13px; border: 1px solid var(--border2); background: var(--sand); border-radius: var(--r); cursor: pointer; font-family: var(--mono); font-size: .72rem; color: var(--ink2); text-decoration: none; transition: all .15s; white-space: nowrap; font-weight: 500; }
        ...
        /* ── NEW: shared ID bar shown on Track/Claim/Void ── */
        .id-bar {
          display: flex; gap: 8px; align-items: center;
          margin-bottom: 20px;
        }
        .id-bar .input { flex: 1; }
        .id-bar-label { font-family: var(--mono); font-size: .65rem; color: var(--ink3); text-transform: uppercase; letter-spacing: .1em; white-space: nowrap; }

        .strip { display: flex; align-items: center; gap: 10px; padding: 9px 14px; background: var(--sand); border: 1px solid var(--border); border-left: 3px solid var(--orange); border-radius: var(--r); margin-bottom: 28px; font-family: var(--mono); font-size: .7rem; flex-wrap: wrap; }
        ...
        .tab { padding: 10px 24px 10px; border: none; background: transparent; color: var(--ink3); font-family: var(--font); font-size: .82rem; font-weight: 800; font-variation-settings: 'wdth' 105; letter-spacing: .04em; text-transform: uppercase; cursor: pointer; border-bottom: 2px solid transparent; margin-bottom: -2px; transition: all .15s; }
        .tab:hover:not(.active) { color: var(--ink2); }
        .tab.active { color: var(--orange); border-bottom-color: var(--orange); }

        .card { background: #fff; border: 1px solid var(--border); border-radius: var(--r-lg); padding: 26px; margin-bottom: 16px; box-shadow: var(--shadow); }
        .card-head { display: flex; align-items: center; gap: 10px; margin-bottom: 22px; }
        .card-title { font-size: .88rem; font-weight: 900; font-variation-settings: 'wdth' 115; text-transform: uppercase; letter-spacing: .04em; color: var(--ink); }
        .chip { font-family: var(--mono); font-size: .62rem; font-weight: 700; padding: 2px 7px; border-radius: var(--r); letter-spacing: .06em; text-transform: uppercase; }
        ...
        .label { display: block; font-size: .65rem; font-weight: 800; font-variation-settings: 'wdth' 110; color: var(--ink3); margin-bottom: 5px; letter-spacing: .1em; text-transform: uppercase; }
        .input { width: 100%; padding: 10px 12px; border-radius: var(--r); border: 1px solid var(--border2); background: var(--sand); color: var(--ink); font-family: var(--mono); font-size: .78rem; outline: none; transition: border-color .15s, background .15s; }
        .input:focus { border-color: var(--orange); background: #fff; box-shadow: 0 0 0 3px var(--orange-dim); }
        .input::placeholder { color: var(--ink3); }
        .row2 { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }

        .btn { display: inline-flex; align-items: center; justify-content: center; gap: 7px; padding: 11px 20px; border-radius: var(--r); border: none; cursor: pointer; font-family: var(--font); font-size: .8rem; font-weight: 900; font-variation-settings: 'wdth' 110; text-transform: uppercase; letter-spacing: .04em; transition: all .15s; }
        ...
        .deal-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 18px; gap: 12px; flex-wrap: wrap; }
        .deal-id { font-family: var(--mono); font-size: .75rem; color: var(--ink3); margin-bottom: 4px; }
        .deal-amount { font-size: 2.4rem; font-weight: 900; font-variation-settings: 'wdth' 120; letter-spacing: -.06em; color: var(--ink); line-height: 1; }
        ...
        .meta-item { display: flex; align-items: center; gap: 6px; font-family: var(--mono); font-size: .7rem; background: var(--sand); border: 1px solid var(--border); border-radius: var(--r); padding: 5px 10px; }
        ...
        .prog-track { height: 6px; background: var(--sand2); border-radius: 99px; overflow: hidden; margin-bottom: 20px; border: 1px solid var(--border); }
        .prog-fill { height: 100%; background: linear-gradient(90deg, var(--orange), var(--gold)); border-radius: 99px; transition: width .5s ease; }

        .ms-grid { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 20px; }
        .ms-node { width: 36px; height: 36px; border-radius: var(--r); display: flex; align-items: center; justify-content: center; font-family: var(--mono); font-size: .65rem; font-weight: 700; border: 1px solid var(--border); background: var(--sand); color: var(--ink3); transition: all .18s; }
        .ms-node.done { background: var(--teal-dim); border-color: var(--teal)55; color: var(--teal); }

        .sub-label { font-size: .62rem; font-weight: 800; font-variation-settings: 'wdth' 110; text-transform: uppercase; letter-spacing: .1em; color: var(--ink3); margin-bottom: 12px; }
        .desc { font-size: .82rem; color: var(--ink2); line-height: 1.7; margin-bottom: 18px; }

        .card-footer { display: flex; align-items: center; gap: 8px; padding-top: 14px; margin-top: 6px; border-top: 1px solid var(--sand2); font-family: var(--mono); font-size: .68rem; color: var(--ink3); }

        .exlink { display: inline-flex; align-items: center; gap: 3px; color: var(--orange); font-family: var(--mono); font-size: .7rem; text-decoration: none; border-bottom: 1px solid var(--orange)40; padding-bottom: 1px; transition: all .13s; }
        .exlink:hover { color: var(--ink); border-color: var(--ink)66; }

        .toast-stack { position: fixed; bottom: 20px; right: 20px; z-index: 99; display: flex; flex-direction: column; gap: 8px; pointer-events: none; }
        .toast { display: flex; align-items: center; gap: 9px; padding: 10px 14px; border-radius: var(--r-lg); font-family: var(--mono); font-size: .76rem; animation: slideIn .2s ease; max-width: 340px; border: 1px solid; pointer-events: all; box-shadow: var(--shadow-lg); }
        ...
        .toast-msg { flex: 1; }
        .toast-link { display: inline-flex; align-items: center; gap: 3px; color: currentColor; text-decoration: none; border-bottom: 1px solid currentColor; font-size: .68rem; opacity: .75; flex-shrink: 0; }
      ...
      <div className="app">
        ...
        {/* ── NEW: no-wallet banner ── */}
        {!walletPub && <WalletBanner onConnect={connectWallet} />}
        ...
        {/* ── NEW: shared Agreement ID bar for Track / Claim / Void ── */}
        {tab !== "forge" && (
          <div className="id-bar">
            <span className="id-bar-label">Agreement ID</span>
            <input
              className="input"
              placeholder="project_alpha"
              value={agreementId}
              onChange={e => { setAgreementId(e.target.value); setEscrow(null); }}
            />
            {tab === "track" && (
              <button className="btn btn-outline" style={{ marginTop: 0, whiteSpace: "nowrap" }}
                onClick={() => fetchEscrow(agreementId)} disabled={loading}>
        ...
        {tab === "track" && escrow && (
          <div className="card">
            <div className="deal-header">
              <div>
                <div className="deal-id">#{agreementId}</div>
                <div className="deal-amount">{fmt(escrow.total_amount)}<span className="deal-unit">XLM</span></div>
              </div>
            ...
            <div className="ms-grid">
              {escrow.milestones?.map((done, i) => (
                <div key={i} className={`ms-node ${done ? "done" : ""}`} title={`Phase ${i + 1}${done ? " — approved" : ""}`}>
            ...
            <div className="row2">
              ...
              <div className="field" style={{ display: "flex", alignItems: "flex-end" }}>
                {/* ── CHANGED: disable Approve if selected phase already done ── */}
                <button className="btn btn-teal btn-full" style={{ marginTop: 0 }}
                  onClick={completeMilestone}
                  disabled={loading || !walletPub || (milestoneIdx !== "" && escrow?.milestones?.[parseInt(milestoneIdx)] === true)}>
                  {loading ? <IconSpin /> : <IconCheck />}
                  {milestoneIdx !== "" && escrow?.milestones?.[parseInt(milestoneIdx)] ? "Already Approved" : "Approve"}
                </button>
              </div>
            </div>
        ...
        {tab === "claim" && (
          <div className="card">
            ...
            <button className="btn btn-teal btn-full" onClick={releaseEscrow} disabled={loading || !walletPub || !agreementId}>
        ...
        {tab === "void" && (
          <div className="card">
            ...
            <button className="btn btn-danger btn-full" onClick={cancelEscrow} disabled={loading || !walletPub || !agreementId}>
